...
public class TestDeAgencia {
	...
	@Test
	public void cargaDeUsersTest() throws IOException {
	...
	public void CargaDeAtraccionesTest() throws IOException {
		...
		assertTrue(a1.listaDeAtracciones.get(1).getTipoDeAtraccion().equals(tipoDeAtraccion.AVENTURA));
		assertTrue(a1.listaDeAtracciones.get(8).getTipoDeAtraccion().equals(tipoDeAtraccion.PAISAJE));
	...
	public void CargaDePromosTest() throws IOException {
		...
		assertEquals(a1.listaDePromociones.get(2).getPrecio(), 30 ,0 ); //TEST ABSOLUTA
		assertEquals(a1.listaDePromociones.get(4).getPrecio(), 114.4 , 0 ); //TEST PORCENTUAL
		//Comprobación de una promoción al azar, que sus atracciones correspondan a la lista principal
		assertTrue(a1.listaDeAtracciones.contains(a1.listaDePromociones.get(5).getAtraccionesContenidas().get(0)));
		assertTrue(a1.listaDeAtracciones.contains(a1.listaDePromociones.get(3).getAtraccionesContenidas().get(1)));
		
	}

	
	@Test
	public void filtradoTest() throws IOException {
		a1.LeeYcargaUsuarios("TestUsuarios.txt");
		a1.LeeYcargaAtracciones("TestAtracciones.txt");
		a1.mapaAtraccionPorNombre();
		a1.LeeYcargaPromociones("TestPromociones.txt");
		a1.filtroSugerencias();
		//Comprueba de otorgar la unica atracción que puede comprar al usuario con menor dinero y tiempo
		assertTrue(a1.listaDeUsuarios.get(7).getSugerenciasDiarias().contains(a1.listaDeAtracciones.get(12)));
		//Comprueba de que el usuario con más tiempo y dinero contiene todas las atracciones
		assertTrue(a1.listaDeUsuarios.get(6).getSugerenciasDiarias().containsAll(a1.listaDeAtracciones));
